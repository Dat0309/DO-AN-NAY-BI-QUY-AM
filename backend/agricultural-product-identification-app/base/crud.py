...
class BaseCrud(metaclass=SingletonMeta):
    ...
    async def add_many(self,data:list,session=None):
        result = None
        if self.key != '_id':
            await self.db.mongodb[self.model].create_index(self.key, unique=True)
        try:
            result = await self.db.mongodb[self.model].insert_many(data,session=session)
        except Exception as e:
            raise HTTPException(status_code=422, detail=f'{e}')
    ...
    async def information(self,total,pages_size,page,limit):
        return PaginationInfo(total=total,pages_size=pages_size,page=page,limit=limit)
    ...
    async def get_all(self, **kwargs):
        ...
        if is_get_info == True:
            ...
            info = await self.information(total, pages_size, page, limit)
